...
from tts_preparation.core.data import (DatasetType, PreparedDataList,
                                       get_speaker_wise)
from tts_preparation.core.helper import (prep_data_list_to_dict_with_durations,
                                         prep_data_list_to_dict_with_symbols,
...
def __add(existing_set: PreparedDataList, restset: PreparedDataList, symbols: SymbolIdDict, func: Callable[[OrderedDictType[int, List[str]], SymbolIdDict], OrderedSet[int]], **kwargs) -> Tuple[PreparedDataList, PreparedDataList]:
  ...
  speaker_data = get_speaker_wise(restset)

  for speaker_id, speaker_available in speaker_data.items():
    speaker_data_dict = prep_data_list_to_dict_with_symbols(speaker_available, symbols)

    selected_keys = func(
      speaker_available=speaker_available,
      speaker_data_dict=speaker_data_dict,
      **kwargs
    )

    not_selected_keys = set(speaker_data_dict.keys()).difference(selected_keys)
    ...
    logger.info(
      f"Took {len(selected_data)}/{len(speaker_available)} utterances from speaker {speaker_id} ({selected_data.get_total_duration_s()/60/60:.2f}h).")
...
def __add_random_percent(speaker_available: PreparedDataList, speaker_data_dict: OrderedDictType[int, List[str]], seed: int, percent: float):
  return random_percent(speaker_data_dict, seed, percent)
...
def __add_symbols(speaker_available: PreparedDataList, speaker_data_dict: OrderedDictType[int, List[str]], cover_symbols: Set[int]):
  return cover_symbols_default(speaker_data_dict, cover_symbols)
...
def __add_random_ngram_cover_seconds(speaker_available: PreparedDataList, speaker_data_dict: OrderedDictType[int, List[str]], n_gram: int, ignore_symbols: Optional[Set[str]], seed: int, seconds: float):
  speaker_avail_durations = prep_data_list_to_dict_with_durations(speaker_available)
  return random_ngrams_cover_seconds(speaker_data_dict, n_gram, ignore_symbols, seed, speaker_avail_durations, seconds)
...
def __add_greedy_ngram_seconds(speaker_available: PreparedDataList, speaker_data_dict: OrderedDictType[int, List[str]], n_gram: int, ignore_symbols: Optional[Set[str]], seconds: float):
  speaker_avail_durations = prep_data_list_to_dict_with_durations(speaker_available)
  return greedy_ngrams_seconds(speaker_data_dict, n_gram, ignore_symbols, speaker_avail_durations, seconds)
...
def __add_greedy_kld_ngram_seconds(speaker_available: PreparedDataList, speaker_data_dict: OrderedDictType[int, List[str]], n_gram: int, ignore_symbols: Optional[Set[str]], seconds: float):
  speaker_avail_durations = prep_data_list_to_dict_with_durations(speaker_available)
  return greedy_kld_uniform_ngrams_seconds(speaker_data_dict, n_gram, ignore_symbols, speaker_avail_durations, seconds)
